...
function embedded() {
	...
	return {
		...
		[Symbol.iterator]() {
			return Object.keys(idx)[Symbol.iterator]();
		}
	};
}
function patch(id, bundle, exports) {
	...
	function readFileSync(path, opts, ...rest) {
		if (!path.startsWith(PROTO)) return exports.readFileSync.call(this, path, opts, ...rest);
		...
		return Object.create(exports, {
			...
			readdirSync: {
				value: function readdirSync(path, opts) {
					if (!path.startsWith(PROTO)) return exports.readdirSync.call(this, path, opts);
			...
			readFile: {
				value: function (path, opts, cb) {
					if (!path.startsWith(PROTO)) return exports.readFile.call(this, path, opts, cb);
					if (!cb) {
						cb = opts;
						opts = undefined;
					}
		...
		return Object.create(exports, {
			...
			exists: {
				value: function exists(path) {
					if (!path.startsWith(PROTO)) return exports.exists.call(this, path);
					path = resolve(path).slice(PROTO.length);
...
function loader(bundle) {
	...
	process.argv[0] = `sea:${resolutions['<main>']['<main>']}`;
	...
	process.addon = function addon(baseurl) {
		if (!baseurl.startsWith('sea:')) throw new Error('not inside sea');
		const basename = baseurl.slice(PROTO.length);
		const addons = Array.from(bundle).filter(name => (name.startsWith(basename) && name.endsWith('.node'))).sort((a, b) => a.length - b.length);
		return load(baseurl, 'sea:' + addons[0]);
	};
	return () => load(null, null);
}

const main = loader(embedded());
main();
